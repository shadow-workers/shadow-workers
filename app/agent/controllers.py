...
def geturl():
    ...
    ConnectedAgents[agentID]['user-agent'] = safeParam(request.headers.get('User-Agent'))
...
def updateAgent(agentID, params):
    ...
    if agent is None:
        agent = Agent(agentID, now, now, params['domain'], params['port'], params['ip'], params['user-agent'])
